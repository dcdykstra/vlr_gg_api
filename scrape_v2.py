...
# OPEN THESE TO START WORKING
# https://selectolax.readthedocs.io/en/latest/parser.html#node
# https://github.com/axsddlr/vlrggapi/blob/master/api/scrape.py
...
class VLR_Scraper():
    ...
    def get_event_urls(self, url, event_type="completed"):
    ...
    def get_match_stats_header(self, url):
        html, status = self.get_html(url)
        match_stats = []
    ...
    def get_match_stats_round_order(self, url):
        html, status = self.get_html(url)
        match_stats = []
        container = html.css_first("div.vm-stats-container")
        games = container.css("div.vm-stats-game:not([data-game-id=all])")
    ...
    def get_match_stats_players(self, url, side="both"):
        html, status = self.get_html(url)
        match_stats = []
        container = html.css_first("div.vm-stats-container")
        games = container.css("div.vm-stats-game:not([data-game-id=all])")
